...
f = lambda c: 100.*c**2*(1.-c)**2
dfdc = lambda c: 200.*c*(2.*c**2 - 3.*c  + 1.)
d2fdc2 = lambda c: 200.*(6.*c**2 - 6.*c + 1.)
...
class CahnHilliardJacobianMatrix(lyza.MatrixAssembler):
    ...
    def calculate_element_matrix(self, cell):
        ...
        for idx in range(len(W_arr)):
            ...
            A += 1./DT*np.einsum('i, j -> ij', N, N)*DETJ*W
            B_ += np.einsum('ab, lb, ia -> il', M, B, B)*DETJ*W
            D += -np.einsum('ab, lb, ia -> il', alpha, B, B)*DETJ*W \
...
class CahnHilliardResidualVector(VectorAssembler):
    ...
    def calculate_element_vector(self, cell):
        ...
        for idx in range(len(W_arr)):
            ...
            c_contrib = (C-CN)/DT*N*DETJ*W \
                        + np.einsum('ab,b,ia->i', M, GRADMU, B)*DETJ*W
            c += c_contrib.reshape(c.shape)

            f_contrib = -np.einsum('ab,b,ia->i', alpha, GRADC, B)*DETJ*W \
                        + (MU-dfdc(C))*N*DETJ*W
