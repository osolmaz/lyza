...
def isotropic_elasticity_tensor(i, j, k, l):
    ...
    return lambda_ * delta(i, j) * delta(k, l) + mu * (
...
reshaped_tensor = reshaped_tensor.subs([(lambda_, lambda_val), (mu, mu_val),])

A_11, A_12, A_21, reshaped_tensor_symmetric = partition_matrix_sympy(
    reshaped_tensor, [], res_dofmap=[0, 4, 8, 1, 5, 2]
)

# pprint(reshaped_tensor_symmetric)

A_11, A_12, A_21, A_22 = partition_matrix_sympy(
    reshaped_tensor_symmetric, [0, 1, 3], res_dofmap=[2, 4, 5]
)
